<template>
  <div class="bg-neutral-300 p-8">
    <Lynx-Title text="Atribuir receita a uma empresa" />
    <form class="flex flex-col" @submit.prevent="onSubmit">
      ...
      <label class="flex flex-col w-full pb-8">
        <Lynx-Label label="Receita"/>
        <a class="text-primary-600 font-bold underline" :href="receita.url" target="_blank">{{ receita.nome }}</a>
      </label>
    ...
    onSubmit() {
      ...
      createReceitaRequest(data)
        .then(() => {
          ...
          this.$emit('createdReceita', this.receita)
        })
        .catch(err => this.$snack.error(err.response.data.message || err.message))
        .finally(() => {
          this.loadingButton = false
          this.closeReceitaDetail()
        })
    }
  }
}
