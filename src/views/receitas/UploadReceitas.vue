<template>
    <div>
        <Lynx-Title text="Upload de Prolabores"/>
...
<script>
    ...
    import { uploadReceitasRequest } from "@/services/requests/receitas";
        ...
        methods: {
            upload(files) {
                ...
                this.base64Files(files).then(files => {
                    const receitas = files.map(({name, base64}) => ({ name, base64, competencia: this.getCompetenciaDate }));
                    uploadReceitasRequest({ receitas })
                        .then(response => {
                            this.loading = false;
