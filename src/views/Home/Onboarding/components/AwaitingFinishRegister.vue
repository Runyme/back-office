      ...
      <div class="pt-2 flex flex-col items-center w-full">
        ...
        >
          <template slot-scope="{row}">
            ...
            <b-table-column label="Nome" field="name" sortable>
              <div class="truncate w-64">{{ row.razao_social ? row.razao_social : row.nome_fantasia }}</div>
            </b-table-column>
...
<script>
import {newSalesAwaitingRegisterRequest} from "@/services/requests/charts";
...
export default {
  ...
  methods: {
    momentIntance,
    fetchDataSet() {
      newSalesAwaitingRegisterRequest()
        .then(res => {
