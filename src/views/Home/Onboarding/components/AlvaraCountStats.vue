<template>
  <div class="bg-white p-5 rounded w-full flex items-center justify-around hover:shadow-lg transition relative">
    ...
    <div>
      <span class="text-2xl text-neutral-700">Emissão de Alvarás</span>
    </div>
...
<script>
import {quantityAlvaraRequest} from "@/services/requests/charts";
import moment from "moment";

export default {
  name: "AlvaraCountStats",
  data() {
    return {
      dataSet: {},
...
onboarding_search(){
  ...
  methods: {
    moment,
    fetchData() {
      quantityAlvaraRequest(this.onboarding_search).then(response => this.dataSet = response.data)
    }
  }
}
</script>
