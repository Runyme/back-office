  ...
  <div>
    ...
    <div class="w-full flex justify-between items-start">
      ...
      <div class="flex flex-row-reverse items-end mb-4">
        <b-button class="is-primary ml-4" icon-right="refresh"
                  @click.native="() => {fetchEmpresasWithGuias(); fetchUploadReport()}">Atualizar Lista
        </b-button>
      ...
      <div class="flex flex-row-reverse items-end mb-4 w-1/3">
        <div class="control ml-4">
          <b-taglist attached>
            ...
            <b-tag type="is-warning" size="is-medium">{{ uploadReport.total || '...' }}</b-tag>
          </b-taglist>
        </div>
        <div class="control">
          <b-taglist attached>
            <b-tag type="is-primary" size="is-medium">Erros:</b-tag>
            <b-tag type="is-danger" size="is-medium">{{ uploadReport.withError || '...' }}</b-tag>
          </b-taglist>
        </div>
      </div>
    </div>
      ...
      <template slot-scope="props">
        ...
        <b-table-column field="guias">
          <div class="flex flex-wrap -mb-2">
            <div class="py-1 px-2 bg-neutral-300 rounded mr-2 mb-2 text-xs" v-for="(guia, key) in props.row.guias"
                 :key="key" v-text="guia.tipo"></div>
            <div class="py-1 px-2 bg-neutral-300 rounded mr-2 mb-2 text-xs" v-show="props.row.pagamento_cartao">
              <b-icon size="is-small" type="is-success" icon="credit-card"></b-icon>
            </div>
          </div>
        </b-table-column>
        <b-table-column field="financeiro">
          <b-switch
            ...
            @input="changeLiberacao(props.row, 'financeiro_departamento_liberacao')"
        ...
        <b-table-column field="contabilidade">
          <b-switch
            ...
            @input="changeLiberacao(props.row, 'contabilidade_departamento_liberacao')"
            :disabled="!hasPermission('liberar guias manualmente')"
          />
        </b-table-column>
        <b-table-column field="status">
          <b-tag :type="props.row.guia_liberacao | statusType">{{ props.row.guia_liberacao | dataEnvio }}</b-tag>
        </b-table-column>
    ...
    <b-modal :active.sync="guiasModal" @close="closeGuiasModal" v-if="guiasModal">
      <GerenciamentoGuiasDetails :current-empresa="currentEmpresa" @estornar="fetchEmpresasWithGuias"/>
    </b-modal>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    dataCompetenciaComputed() {
      return this.dataCompetencia ? formatBrToEn(this.dataCompetencia) : this.getCompetenciaDate;
    },
    empresasComputed() {
      ...
      const empresasFilteredList = this.empresas.filter(empresa => {
        ...
        if (carteira_filter === 'sem_carteira') return !empresa.carteiras || !empresa.carteiras.length

        return empresa.carteiras ? empresa.carteiras.some(carteira => carteira.id === carteira_filter) : false
      });

      return empresasFilteredList.filter((data) => {
        switch (filter_status) {
          case 1:
            return data.guia_liberacao.length && data.guia_liberacao[0].data_envio != null && data.guias.length > 0
          case 2:
            return data.guia_liberacao.length && data.guia_liberacao[0].erro_envio
          case 3:
            return !data.guia_liberacao.length || !data.guia_liberacao[0].financeiro_departamento_liberacao
          case 4:
            return !data.guia_liberacao.length || !data.guia_liberacao[0].rh_departamento_liberacao
          case 5:
            return !data.guia_liberacao.length || !data.guia_liberacao[0].contabilidade_departamento_liberacao
          case 6:
            return !data.guia_liberacao.length || !data.guia_liberacao[0].data_envio
          default:
            return true
        }
      });
  ...
  filters: {
    liberado_contabilidade: val => (val && val.length > 0) ? val[0].contabilidade_departamento_liberacao : false,
    liberado_rh: val => (val && val.length > 0) ? val[0].rh_departamento_liberacao : false,
    liberado_financeiro: val => (val && val.length > 0) ? val[0].financeiro_departamento_liberacao : false,
    liberado: val => {
      if (!val || !val.length) return false;
      return val[0].rh_departamento_liberacao && val[0].contabilidade_departamento_liberacao
    },
    dataEnvio: val => {
  ...
  mounted() {
    ...
    this.uploadReportInterval = setInterval(() => this.fetchUploadReport(), 15000)
  },
  destroyed() {
  ...
  methods: {
    ...
    fetchCarteiras() {
      carteirasRequest()
        .then(response => {
          ...
          this.carteiras = data.filter(carteira => carteira.setor === 'contabilidade')
        })
    ...
    changeLiberacao(empresa, departamento) {
      ...
      if (!(guiaLiberacao && guiaLiberacao.length && guiaLiberacao[0][departamento])) {
    ...
    onSendGuia(empresa) {
      const guiaLiberacao = empresa.guia_liberacao && empresa.guia_liberacao.length > 0 ? empresa.guia_liberacao[0] : null
    ...
    validateLiberacao(guiaLiberacao) {
      if (!guiaLiberacao) {
        this.$snack.error('Por favor aguarde a liberação para enviar as guias.')
        return false
      }
    ...
    handleLiberacaoTodas() {
      this.$buefy.dialog.confirm({
        ...
        message: 'Esta ação irá liberar para envio conforme as guias que estão anexadas. Deseja continuar?',
        confirmText: 'Sim',
        cancelText: 'Cancelar',
        type: 'is-primary',
        hasIcon: true,
    ...
    handleEnvioTodas() {
      this.$buefy.dialog.confirm({
        title: 'Liberação de guias',
        message: 'Esta ação irá enviar todas as guias liberadas. Deseja continuar?',
        confirmText: 'Sim',
        cancelText: 'Cancelar',
        type: 'is-primary',
        hasIcon: true,
