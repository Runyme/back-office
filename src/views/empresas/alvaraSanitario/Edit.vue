...
<script>
import {
  storeAlvarasSanitarioRequest,
  updateAlvarasSanitarioRequest,
  showAlvarasSanitarioRequest,
} from "@/services/requests/alvarasSanitario";
...
export default {
  ...
  methods: {
    fetchFile() {
      ...
      if(this.alvaraSan != null){
    showTaxasRequest(this.alvaraSan.id)
        .then((response) => {
      ...
      showAlvarasSanitarioRequest(this.alvaraSan.id)
        .then((response) => {
 ...
 if(!this.arquivo){
      ...
      if (this.alvaraSan == null) {
        ...
        storeAlvarasSanitarioRequest(form)
          .then(() => {
            this.$emit("edit-alvaraSan");
            this.$snack.success("Alvará Sanitario criado com sucesso");
          })
          .catch((err) =>
        ...
        updateAlvarasSanitarioRequest(this.alvaraSan.id, form)
          .then(() => {
            this.$emit("edit-alvaraSan");
            this.$snack.success("Alvará Sanitario atualizado com sucesso");
          })
          .catch((err) =>
            this.$snack.error(err.response.data.message || this.errorMessage)
          )
  ...
  beforeMount() {
    ...
    this.form = {
      empresa_id: this.empresaId,
      data_vencimento: new Date(moment(this.alvaraSan.data_vencimento)),
    };
       }else{
            this.form= {
        empresa_id: this.empresaId,
