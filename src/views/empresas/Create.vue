<template>
  <div>
    <Lynx-Title text="Cadastrar uma nova Empresa"/>
    <Lynx-OverlaySpinner v-if="loading" :loading="loading"/>

    <form class="w-full flex flex-col pb-20" @submit.prevent="submitForm">
      <!-- TIPO -->
      <div class="mb-8">
        <b-field>
          <b-radio-button
            v-model="form.tipo"
            native-value="abertura"
          >
            <span>Abertura</span>
          </b-radio-button>
        ...
        <ul v-if="socios.length" class="py-2 mb-2">
          ...
          >
            <span class="w-full">
              ...
              <b-tooltip v-if="socio_administrador !== socio.cliente.id" label="Remover como sócio" position="is-bottom" type="is-danger">
                <button
      ...
      <div class="mb-8">
        ...
        <label class="block">
          <Lynx-Label
            label="Nome Fantasia"
            :validator="$v.form.nome_fantasia"
            :validations-message="validationsMessage.form.nome_fantasia"
          />

          <Lynx-Input
            placeholder="Insira o Nome Fantasia"
            :error="$v.form.nome_fantasia.$error"
            v-model.trim="$v.form.nome_fantasia.$model"
          />
        </label>

        <div v-if="this.form.tipo == 'transferencia'" class="block flex flex-wrap">
          <Lynx-Label
            label="Razão Social"
            class="mr-2"
          />
        ...
        <div v-if="this.form.tipo == 'abertura'" class="block flex flex-wrap">
          ...
          >
            ...
            <button
              v-if="index != 0"
              class="pl-2 flex items-center justify-center flex-shrink-0 text-neutral-500"
        ...
        <div class="flex">
          ...
          <b-checkbox class="block flex-shrink-0 pt-2" v-model="form.clinica_fisica">Possui Clinica Física</b-checkbox>
        </div>

        <label v-if="this.form.tipo == 'transferencia'" class="block">
          <Lynx-Label
            label="Competência inicial da cobrança"
          />

          <InputCompetenciaDate
            placeholder="Selecione o mês"
            v-model="form.data_inicio_cobranca"
            @input="setDataInicioCobranca"
          />
        </label>
        ...
        <Input-File text="Cartão CNPJ" ref="cartaoCnpjInput" v-if="this.form.tipo == 'transferencia'"/>
      </div>
      ...
      <div class="mb-8 flex flex-wrap">
        ...
        <div class="flex w-full pb-4">
          <small class="text-neutral-600">Atenção! Contatos não "Notificáveis" não receberão o contrato, guias ou
            qualquer outra informação relevante para proceguir como processo.</small>
        </div>
      ...
      <div class="mb-8">
        ...
        <div v-if="loadingPlansError !== null">
          <b-notification :message="'Erro no carregamento dos planos: '+loadingPlansError" class="opacity-50"
                          type="is-danger"/>
        </div>
        <div
      ...
      <Lynx-Button type="submit" text="Criar nova empresa" icon="Plus" class="self-end" :loading="loading"
                   :disable="loading"/>
    </form>
...
import {empresasStoreRequest} from '@/services/requests/empresas'
  ...
  data() {
    return {
      ...
      form: {
        ...
        cnpj: '',
        data_inicio_cobranca: moment().add(1, 'month').toDate(),
        nome_fantasia: '',
        razao_social: [''],
        regime_tributario: '',
        tipo_societario: '',
  ...
  methods: {
    ...
    async submitForm() {
      ...
      if (this.form.tipo == 'abertura' && this.form.razao_social.length < 1) {
        ...
        return
      } else if (this.form.tipo == 'transferencia' && this.form.razao_social.length != 1) {
      ...
      const comprovanteResidenciaId = await this.$refs.comprovanteDeResidenciaInput.submit()
      ...
      const data = {
        precadastro: {
          tipo: this.form.tipo,
          data_inicio_cobranca: this.form.tipo == 'transferencia' ? this.form.data_inicio_cobranca : null,
          empresa: {
            razao_social: this.form.tipo == 'abertura' ? this.form.razao_social : this.form.razao_social,
        ...
        empresa: {
          ...
          razao_social: this.form.tipo == 'transferencia' ? this.form.razao_social[0] : null,
    ...
    setDataInicioCobranca(value) {
      this.form.data_inicio_cobranca = value
    },
  },
}
</script>
