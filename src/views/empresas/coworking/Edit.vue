<template>
  <div class="p-8 bg-neutral-300">
    <div class="flex flex-col">
      <Lynx-Title :text="`Editar Coworking - ${empresaId}`" />
...
<script>
import {
  storeCoworkingRequest,
  updateCoworkingRequest,
  showCoworkingRequest,
} from '@/services/requests/coworking'
...
export default {
  ...
  data() {
    return {
      ...
      errorMessage: "Não foi possivel editar o Coworking",
    };
  },
  computed: {
    ...mapGetters(["getToken"]),
  ...
  methods: {
    fetchFile() {
      ...
      showCoworkingRequest(this.bombeiro.id)
        .then((response) => {
    ...
    async onSubmit() {
      ...
      if ( this.bombeiro == null) {
        storeCoworkingRequest(form)
          .then(() => {
           this.$emit('create-alvara')
            this.$snack.success("Coworking criado com sucesso");
          })
          .catch((err) =>
        ...
        updateCoworkingRequest(this.bombeiro.id, form)
          .then(() => {
            this.$emit('create-alvara')
